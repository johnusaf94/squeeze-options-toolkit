...
def normalise_short_interest(info: dict, ticker: str = "",
    ...
    # ── PREFER THE EXCHANGE'S OWN NUMBERS WHERE THEY EXIST ──
    # Everything above computes DTC as (settlement snapshot) / (rolling 10-day
    # volume), which mixes vintages: the numerator is frozen between settlements
    # while the denominator moves daily, so DTC falls whenever volume rises —
    # exactly when a squeeze begins. NASDAQ publishes, per settlement, the short
    # interest AND the average daily volume over that same period AND the days
    # to cover, all contemporaneous. Where available that is strictly better,
    # and it is free.
    #
    # Nasdaq-listed only: about 75% of this scanner's universe. NYSE names
    # (GME, AMC, HIMS...) keep the computed value, and data_quality says which
    ...
    try:
        ...
        official = {}
    if official and official.get("dtc"):
        result['days_to_cover'] = round(float(official["dtc"]), 2)
        result['dtc_source'] = 'nasdaq_official'
        result['si_settlement'] = str(official.get("settlement", ""))
        result['si_age_days'] = official.get("age_days")
        result['si_avg_volume'] = official.get("avg_volume")
        if official.get("interest"):
            result['shares_short'] = int(official["interest"])
            if float_shares and float_shares > 0:
                recalc = official["interest"] / float_shares
                if 0 <= recalc <= 2.0:
                    result['short_pct_float'] = recalc
                    result['data_quality'] = 'nasdaq_official'
        if official.get("interest_change_pct") is not None:
            result['short_change_pct'] = official["interest_change_pct"]
        # Carry the full settlement series so the caller can build DTC on a
        # named volume window and read trends on the cadence short interest
        # actually has, without paying for a second fetch.
        result['settlement_history'] = official.get("history") or []
    else:
        result['dtc_source'] = 'computed_mixed_vintage'
        result['settlement_history'] = []
...
def _fetch_ftd_period(url: str) -> Optional[dict]:
    """
    Download + parse ONE SEC FTD period file.

    Returns {TICKER: (settlement_date_str, balance_shares)} — the MOST RECENT
    settlement date's fail balance per ticker in the file.

    Why latest-balance, not sum: each SEC row is the aggregate fails-to-deliver
    balance OUTSTANDING as of that settlement date, not new fails that day. The
    same fails carry across consecutive days until they clear, so summing daily
    rows double-counts. The correct per-period figure is the latest daily
    balance (or the full daily series), never the sum.
    """
    ...
    try:
        ...
        qty_col  = next((c for c in df.columns if 'QUANTITY' in c or 'SHARES' in c or 'TOTAL' in c), None)
        date_col = next((c for c in df.columns if 'SETTLEMENT' in c or 'DATE' in c), None)
        if not sym_col or not qty_col:
            _FTD_FILE_CACHE[url] = None
            return None

        period_map = _aggregate_latest_balance(df, sym_col, qty_col, date_col)
...
def _aggregate_latest_balance(df, sym_col, qty_col, date_col):
    """
    Core aggregation, factored out so it can be unit-tested without network.
    Returns {TICKER: (settlement_date_str_or_None, latest_balance_int)}.
    """
    import pandas as pd
    ...
    if date_col:
        # SEC settlement dates are YYYYMMDD strings -> lexical max == chronological max
        df['_DATE'] = df[date_col].astype(str).str.strip()
        df = df.sort_values('_DATE')
        latest = df.groupby('_SYM').tail(1)   # last row per ticker == max date
        return {
            row['_SYM']: (row['_DATE'], int(row[qty_col]))
            for _, row in latest.iterrows()
            if row[qty_col] and not pd.isna(row[qty_col])
        }

    grouped = df.groupby('_SYM')[qty_col].max()
    return {k: (None, int(v)) for k, v in grouped.items()
...
def _ftd_period_urls(n_periods: int = 6) -> list:
    ...
    urls = []
    now = datetime.now()
    # Exact month arithmetic (the old 30-day-step walk drifted near month
    # boundaries and could skip or duplicate a month). Start one month back —
    # current month's file isn't published yet.
    yr, mo = now.year, now.month
    for _ in range((n_periods // 2) + 3):
        mo -= 1
        if mo == 0:
            mo, yr = 12, yr - 1
...
def fetch_sec_ftd(ticker: str, n_periods: int = 6) -> Optional[dict]:
    ...
    Returns:
      ftd_shares        - latest daily fail balance (newest period)
      ftd_series        - list of (period_tag, settlement_date, shares) oldest->newest
      ftd_total_recent  - DEPRECATED (None): summing daily balances is invalid
      ftd_trend         - RISING / FALLING / FLAT (slope over latest balances)
      ftd_periods       - how many periods had data
      report_date       - REAL latest settlement date (YYYYMMDD) for THIS ticker
    ...
    try:
        ...
        series = []   # (period_tag, settlement_date, shares) newest->oldest
        ...
        for tag, url in periods:
            ...
            if entry is not None:
                sdate, shares = entry
                series.append((tag, sdate, shares))
        ...
        series_chrono = list(reversed(series))          # oldest -> newest
        result['ftd_series']   = series_chrono
        result['ftd_shares']   = series[0][2]            # newest period, latest daily balance
        result['ftd_periods']  = len(series)
        result['report_date']  = series[0][1]            # REAL settlement date, per-ticker
        result['data_quality'] = 'live' if len(series) >= 3 else 'partial'
        # ftd_total_recent intentionally left None -- do not sum daily balances.

        if len(series_chrono) >= 2:
            ys = [float(s) for _, _, s in series_chrono]
            n  = len(ys)
            xs = list(range(n))
            mx = sum(xs) / n
            my = sum(ys) / n
...
def fetch_validated_info(ticker: str, enrich: bool = True) -> dict:
    ...
    # ── DAYS TO COVER, BY NAMED VOLUME WINDOW ──
    # A single DTC number is meaningless without saying which volume window
    # produced it: the same settlement yields 5.31 on the exchange's 10-session
    # mean and 13.18 on a 20-session median (GME, 2026-08-14). Both are
    # arithmetically right. dtc_engine reports the family, reproduces the
    # exchange figure as a check that our arithmetic is not the problem, and
    # reads short-interest trends off the settlement series rather than off
    # local snapshots that are the same settlement 74% of the time.
    #
    # `shortRatio` is LEFT ALONE (still the exchange/official figure) so
    ...
    try:
        ...
        if _vol is not None and _hist_rows:
            dtc_data = dtc_engine.for_ticker(ticker, history=_hist_rows,
                                             volumes=_vol)
        elif _hist_rows:
            dtc_data = {'panel': {}, 'trends':
                        dtc_engine.settlement_trends(_hist_rows)}
    except Exception:
        dtc_data = {}
    ...
    # close-out window. effective_float.py nets those out, handling the two
    # arithmetic traps that make the naive version wrong (ownership percentages
    # are of shares OUTSTANDING, and reported float has already removed insider
    # stock). Reported only — no scoring gate reads these.
    eff_data = {}
    ftd_pct_eff = None
    try:
        if not enrich:
        ...
        eff_data = compute_effective_float(
            ...
            shares_out,
            safe_float(raw, 'heldPercentInsiders'),
            safe_float(raw, 'heldPercentInstitutions'))
        _ef = eff_data.get('effective_float')
        _fs = ftd_data.get('ftd_shares')
        if _ef and _ef > 0 and _fs:
            ftd_pct_eff = _fs / _ef
    except Exception:
        eff_data = {}
    ...
    result = ValidatedDict({
        ...
        # Days to cover by named volume window (shortRatio above is unchanged)
        'dtcExchange':         _panel.get('exchange'),
        'dtcRobust':           _panel.get('robust_10d_median'),
        'dtc20d':              _panel.get('horizon_20d_median'),
        'dtc60d':              _panel.get('horizon_60d_median'),
        'dtcPreferred':        _panel.get('preferred'),
        'dtcPreferredBasis':   _panel.get('preferred_basis', ''),
        'dtcSpreadLow':        _panel.get('spread_low'),
        'dtcSpreadHigh':       _panel.get('spread_high'),
        'dtcSpikeRatio':       _panel.get('spike_ratio'),
        'dtcSpikeContaminated': _panel.get('spike_contaminated', False),
        'dtcReplicaError':     _panel.get('replica_error'),
        'dtcNotes':            _panel.get('notes', []),

        # Trends on the settlement cadence, not on 7-day local snapshots
        'siTrendSettlement':   _trends.get('si_trend', ''),
        'siChangeSettlement':  _trends.get('si_change_pct'),
        'dtcTrendSettlement':  _trends.get('dtc_trend', ''),
        'settlementDate':      (str(_trends.get('settlement'))
                                if _trends.get('settlement') else ''),
        'settlementAgeDays':   _trends.get('age_days'),
        'settlementCount':     _trends.get('n_settlements', 0),
        'settlementConsecutive': _trends.get('consecutive', 0),
        'settlementNotes':     _trends.get('notes', []),
        'dtcMoveIsLiquidity':  _trends.get('dtc_move_is_liquidity', False),
        'settlementVolChange': _trends.get('volume_change_pct'),
        ...
        'effectiveFloatQuality': eff_data.get('quality'),
